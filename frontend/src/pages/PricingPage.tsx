    ...
    <div className="mx-auto max-w-4xl space-y-6 px-4 py-8 text-gray-200">
      ...
      <section className="relative overflow-hidden rounded-[1.85rem] border border-white/8 bg-[radial-gradient(circle_at_top_left,rgba(56,189,248,0.18),transparent_24rem),linear-gradient(135deg,rgba(15,23,42,0.98),rgba(8,15,30,0.94))] px-5 py-6 shadow-[0_30px_75px_rgba(2,6,23,0.48)] sm:px-6">
        ...
        <p className="mt-3 max-w-3xl text-sm leading-6 text-gray-300">
          Every club gets one free competition. After your free competition is used, each additional competition requires one competition slot at <strong className="text-white">€29</strong>.
        </p>
        <p className="mt-2 max-w-3xl text-sm leading-6 text-gray-300">
          This keeps costs predictable with a low fixed fee per competition, instead of charging a percentage or per-player platform cut.
        </p>
      </section>
      ...
      <section className="grid gap-4 sm:grid-cols-2">
        <article className="card border-brand-400/25 bg-brand-500/10">
          ...
          <h2 className="mt-2 text-3xl font-black text-white">First competition free</h2>
          <p className="mt-2 text-sm leading-6 text-gray-300">Create your first competition at no cost, then continue running more competitions with competition slots.</p>
        </article>
      ...
      <section className="card space-y-4">
        ...
        <div className="rounded-2xl border border-emerald-500/25 bg-emerald-500/10 p-4">
          <p className="text-xs font-black uppercase tracking-[0.16em] text-emerald-200">Low fixed cost</p>
          <p className="mt-1 text-sm text-gray-200">No per-player platform deductions. You pay a fixed €29 to create each additional competition after your free one.</p>
        </div>
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="rounded-2xl border border-brand-400/25 bg-brand-500/10 p-4">
            ...
            <p className="mt-1 text-2xl font-black text-white">€29 per slot</p>
            <p className="mt-1 text-sm text-gray-300">Each additional competition consumes one slot. Slots are one-off purchases, do not expire, and can be stocked up in advance.</p>
          </div>
        </div>
