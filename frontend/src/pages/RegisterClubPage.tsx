      ...
      <nav className="border-b border-white/8 bg-[linear-gradient(180deg,rgba(8,15,30,0.92),rgba(8,15,30,0.78))] backdrop-blur-xl px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4 text-sm text-gray-400">
          ...
          <span>
            ...
            <Link to={`/login?returnTo=${encodeURIComponent('/create-club')}`} className="text-brand-400 hover:text-brand-300 font-medium">Sign in</Link>
          </span>
        </div>
      </nav>
        ...
        <div className="w-full max-w-5xl grid gap-6 lg:grid-cols-[1fr_1.05fr]">
          ...
          <div className="w-full max-w-lg lg:max-w-none mx-auto">
            <div className="text-center mb-8">
              <div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-brand-600/20 border border-brand-500/30 text-3xl mb-4">🏠</div>
              <h1 className="text-3xl font-bold text-white">Register Your Club</h1>
              <p className="text-gray-400 mt-2">Create your account and club together in one setup.</p>
            </div>
              ...
              {[1, 2].map((s) => (
                <div key={s} className="flex items-center gap-2">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${
                    step === s ? 'bg-brand-600 text-white scale-110' : s < step ? 'bg-green-600 text-white' : 'bg-surface-700 text-gray-500'
                  }`}>
                    {s < step ? '✓' : s}
                  </div>
                  <span className={`text-sm hidden sm:block ${step === s ? 'text-white font-medium' : 'text-gray-500'}`}>{s === 1 ? 'Club Details' : 'Your Account'}</span>
            ...
            {step === 1 && (
              <form onSubmit={handleNext} className="card space-y-5">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1.5">Club Name <span className="text-red-400">*</span></label>
                  <input value={clubName} onChange={(e) => setClubName(e.target.value)} className="input-field" placeholder="e.g. St. Nicholas GAA, The Red Lion Pub" required maxLength={80} autoFocus />
                ...
                <div>
                  ...
                  <textarea value={clubDescription} onChange={(e) => setClubDescription(e.target.value)} className="input-field" placeholder="A short description of your club or competition rules..." rows={3} maxLength={300} />
                </div>
                <BenefitsPanel />
              ...
              <form onSubmit={handleSubmit} className="card space-y-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-400">This will be the admin account for <span className="text-white font-medium">"{clubName}"</span></p>
                  </div>
                </div>
                <div className="grid gap-4 sm:grid-cols-2">
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-1.5">Username <span className="text-red-400">*</span></label>
                    <input value={username} onChange={(e) => { setUsername(e.target.value.replace(/\s+/g, '')); setUsernameStatus('idle'); setUsernameMessage(''); }} onBlur={checkUsernameAvailability} className={`input-field ${usernameStatus === 'available' ? 'border-green-500/60 focus:border-green-400' : usernameStatus === 'taken' || usernameStatus === 'error' ? 'border-red-500/60 focus:border-red-400' : ''}`} placeholder="yourname" required minLength={3} maxLength={30} autoFocus />
                    {usernameStatus !== 'idle' && <p className={`mt-1 text-xs ${usernameStatus === 'available' ? 'text-green-400' : usernameStatus === 'checking' ? 'text-gray-400' : 'text-red-400'}`}>{usernameStatus === 'checking' ? 'Checking username...' : usernameMessage}</p>}
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-1.5">Email <span className="text-red-400">*</span></label>
                    <input type="email" value={email} onChange={(e) => { setEmail(e.target.value); setEmailStatus('idle'); setEmailMessage(''); }} onBlur={checkEmailAvailability} className={`input-field ${emailStatus === 'available' ? 'border-green-500/60 focus:border-green-400' : emailStatus === 'taken' || emailStatus === 'error' ? 'border-red-500/60 focus:border-red-400' : ''}`} placeholder="you@example.com" required />
            ...
            <p className="text-center text-sm text-gray-500 mt-6">Just want to play? <Link to="/signup" className="text-brand-400 hover:text-brand-300">Sign up as a player</Link></p>
          </div>
        </div>
      </div>
    </div>
...
function PasswordField({ label, value, onChange, show, onToggle, placeholder, error }: { label: string; value: string; onChange: (value: string) => void; show: boolean; onToggle: () => void; placeholder: string; error?: boolean }) {
  return (
    <div>
      <label className="block text-sm font-medium text-gray-300 mb-1.5">{label} <span className="text-red-400">*</span></label>
      <div className="relative">
        <input type={show ? 'text' : 'password'} value={value} onChange={(e) => onChange(e.target.value)} className={`input-field pr-12 ${error ? 'border-red-500' : ''}`} placeholder={placeholder} required />
        <button type="button" onClick={onToggle} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-white text-sm">{show ? 'Hide' : 'Show'}</button>
      </div>
    </div>
  );
}
    ...
    <div className="rounded-xl bg-brand-600/10 border border-brand-500/20 p-4 space-y-2">
      ...
      <ul className="space-y-1.5">
        {[
          ...
          <li key={item} className="flex items-start gap-2 text-sm text-gray-300"><span className="text-green-400 shrink-0 mt-0.5">✓</span>{item}</li>
        ))}
      </ul>
    </div>
  );
