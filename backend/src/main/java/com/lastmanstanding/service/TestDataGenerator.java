...
public class TestDataGenerator {
    ...
    public GenerationResult generateTestUsers(Long competitionId, int count, List<Integer> gameweeksToSeedPicks) {
        ...
        // Load ALL existing test users in ONE query instead of N existsByEmail checks
        Map<String, User> existingByEmail = userRepository.findAllByOrderByUsernameAsc().stream()
        ...
        participantRepository.saveAll(participantsToCreate);
        int usersJoined = participantsToCreate.size();
        ...
        if (gameweeksToSeedPicks != null && !gameweeksToSeedPicks.isEmpty()) {
            ...
            for (Integer weekNumber : gameweeksToSeedPicks) {
                ...
                pickRepository.saveAll(picksToCreate);
    ...
    public int cleanupTestUsers() {
        ...
        // Load all test users in one query
        List<Long> userIds = userRepository.findAllByOrderByUsernameAsc().stream()
                .filter(u -> u.getUsername().startsWith("testuser"))
