...
public interface PickResultRepository extends JpaRepository<PickResult, Long> {
    ...
    @Modifying
    @Query("DELETE FROM PickResult pr WHERE pr.pick.id IN (SELECT p.id FROM Pick p WHERE p.competition.id = :competitionId AND p.participant.id = :participantId AND p.gameweek.weekNumber > :weekNumber)")
    void deleteFuturePickResultsForParticipant(@Param("competitionId") Long competitionId, @Param("participantId") Long participantId, @Param("weekNumber") int weekNumber);

    @Modifying
    @Query("DELETE FROM PickResult pr WHERE pr.pick.id IN (SELECT p.id FROM Pick p WHERE p.competition.id = :competitionId AND p.participant.id IN :participantIds AND p.gameweek.weekNumber > :weekNumber)")
    void deleteFuturePickResultsForParticipants(@Param("competitionId") Long competitionId, @Param("participantIds") List<Long> participantIds, @Param("weekNumber") int weekNumber);
